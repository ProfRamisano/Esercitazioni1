...
\end{esercizio}
\textit{Soluzione.} Le $13$ colonne vengono compilate inserendo, con ripetizione, i tre simboli disponibili. Il numero di colonne è quindi uguale al numero di disposizioni con ripetizione:  
\[
D'_{13,3}=3^{13}
\] 
...
\end{esercizio}
\textit{Soluzione.} Le lettere della parola \textit{studiare} non sono ripetute. Il numero di anagrammi è uguale a quello delle permutazioni semplici: 
\[
P_{8}=8!
\]
...
k=\frac{10}{3}
\]
Il valore di $k$ non è accettabile in quanto $k$ deve essere intero. 
...
\begin{itemize}
	...
	\sum_{k=0}^n (-1)^k \, {n\choose k}=0 
	\]
	Dallo sviluppo precedente si deduce che la somma dei coefficienti di indice pari è uguale allo somma di quelli con indice dispari: 
	\[
